        ...
        <div class="border-b px-4 pb-6">
            <div class="text-center my-4">
                <img class="h-32 w-32 rounded-full border-4 bg-blue-500 border-white dark:border-gray-800 mx-auto my-4"
                    src="" alt="">
                <div class="py-2">
                    <h3 class="font-bold text-2xl text-gray-800 dark:text-white mb-1">{{user.first_name}}</h3>
...
export default {
  data() {
    return {
      user: { // Single user object for the profile card
        first_name: "John",
        last_name: "Doe",
  ...
  methods: {
    async fetchUsers() {
      ...
      try {
        ...
        // Set the first user as the `user` for the profile card
        if (this.users.length > 0) {
          this.user = this.users[0]; // Example: Display the first user
        }
      } catch (err) {
